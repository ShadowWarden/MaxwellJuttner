...
contains
    ...
    subroutine generate_maxwell_juttner(Npoints,A,u,p_parallel,p_s,p_s_sin)
        integer, intent(in) :: Npoints
        real, intent (in) :: A, u
        real, dimension(:), allocatable, intent (out) :: p_parallel, p_s, p_s_sin
    ...
    !       write(*,*) "Values of qs are ",qps,qmis,qms
        ...
        do while(i<Npoints)
            call random_number(V)
            p_s_sin(i+1) = p_s(i+1)*sin(V*2*pi)
            p_s(i+1) = p_s(i+1)*cos(V*2*pi)
            i = i+1
        end do

        return
    end subroutine generate_maxwell_juttner

end module mod_random_var
!program test_bisect
!    use random_variate
!    
!    integer :: Npoints
...
!    u = 1e-3
!  
!    call generate_maxwell_juttner(Npoints,A,u,p_parallel,p_s,p_s_sin) 
!
